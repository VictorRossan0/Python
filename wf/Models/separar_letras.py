...
pasta_letras = 'letras'
pasta_identificado = 'identificado'
...
arquivos = glob.glob('Images/*')  # Alteração do diretório de origem
...
for arquivo in arquivos:
    ...
    for i, retangulo in enumerate(regiao_letras, 1):
        ...
        cv2.imwrite(f'letras/{nome_arquivo}', imagem_letra)  # Alteração do diretório de destino
        cv2.rectangle(imagem_final, (x, y), (x+largura+5, y+altura+5), (0, 255, 0), 1)
        print(f'letras/{nome_arquivo}')

    nome_arquivo = os.path.basename(arquivo)
    cv2.imwrite(f"identificado/{nome_arquivo}", imagem_final)
    print(f"identificado/{nome_arquivo}")
