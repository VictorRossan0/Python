...
# Configure o logger para salvar mensagens de log em um arquivo
...
def convert_csv_to_excel():
    ...
    try:
        ...
        # Gerar um nome de arquivo único com base no timestamp atual
        timestamp = int(time.time())
        
        # Converter o arquivo CSV para Excel
        excel_file_path = os.path.join(UPLOAD_DIRECTORY, f'converted_file_{timestamp}.xlsx')
        writer = pd.ExcelWriter(excel_file_path)
        df.to_excel(writer, index=False)
        writer.close()  # Fechar o arquivo após a escrita

        flash('Arquivo CSV convertido para Excel com sucesso.', 'success')
        logging.info(f'Arquivo Excel convertido salvo em: {excel_file_path}')
...
def process_excel():
    ...
    try:
        ...
        excel_file_path = os.path.join(UPLOAD_DIRECTORY, f'colunas_convertidas_{timestamp}.xlsx')
        df.to_excel(excel_file_path, index=False)
        
        flash('Colunas convertidas e salvas em um novo arquivo Excel.', 'success')
        
        # Salvar o DataFrame em um arquivo CSV
        csv_file_path = os.path.join(UPLOAD_DIRECTORY, f'dados_convertidos_{timestamp}.csv')
